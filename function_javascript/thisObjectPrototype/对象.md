...
**注意:** 下一部分，我们描述 "属性描述符" (属性特点？ propertie characteristics ),并且展示 `Object.defineProperty(..)`的用法. `Object.assign(..)`中发生的复制仅仅是`=`方式的赋值，所以在来源对象中的所有特殊的属性特点(比如`writable`) **不会保存** 在目标对象里。  ？？？

### 属性描述符

ES5之前，JavaScript 语言没有给出一种直接的方式来检测或者挖掘出属性特点之间的不同，例如属性是否是只读的。
